...
#6 Create Public Ip
resource "azurerm_public_ip" "ExpressRoute_pip" {
  name = "ergw-pip-estaus-001"
  resource_group_name = azurerm_resource_group.rg.name
  location            = var.locations.hub
...
ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.ExpressRoute_pip.id
